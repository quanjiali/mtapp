...
exports.loadmore = async(req, res) => {
    ...
    const restaurants = await Restaurant.find().skip(skipNum).limit(3)
    res.json(restaurants)
}

exports.addRestaurant = async(req, res) => {
    const result1 = req.flash('success', '成功')
...
exports.addRestaurantPost = async(req, res) => {
        ...
        try {
            ...
            const errormsg = res.json(error)
            req.flash('fail', errormsg)
            res.redirect('/add-restaurant')
        }
    }
    //sortRestaurant
...
exports.searchPost = async(req, res) => {
    try {
        ...
        const searchResult = await Restaurant.find({
            $text: {
                $search: keyword
            }
        })
